...
Users may select others in the Keymaps menu, or load a  CSV format keymaps exported from the 
the [official Twiddler layout editor](https://twiddler.tekgear.com/tuner/edit.xhtml).

Menu options include:
* File > Load Keymap imports a CSV file generated by the Twiddler Tuner.
* File > Load Lesson imports a arbitrary ascii text file, displayed one line at a time.
* Keyboard > Show/Hide.. button labels and/or Reverse the keyboard orientation.
* Hints > Highlight... to show/hide buttons for the next letter in the lesson.
* Keymaps > selects keypams: Backspice, Default_V5, TabSpace, and Typemax.
