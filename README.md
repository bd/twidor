...
Labels within the boxes on the keyboard shows the letter
generated by a single-button keystroke.

To the right of each button, are two-button chords. This 2nd level keymap is oriented
the same as the top level button boxes: horizonal rows corresponding to buttons under the index to pinky
finger, and vertical columns of red-blue-green labels corresponding to the red-blue-green columns of buttons.  
